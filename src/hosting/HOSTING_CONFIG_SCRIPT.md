...
3. Proxy and non-SSH network exposure policy remain outside this script. SSH service startup and common host-firewall allowance are partially covered by explicit admin setup.
4. Hardware-backed key storage is not implemented.
5. Local user-account compromise remains outside baseline prevention guarantees.

Transport-related limit details:

| Area | What the script does today | Why the remaining limit exists | Could this script close more? |
| --- | --- | --- | --- |
| Realm-local SSH client artifacts | `--transport-provision-ssh-artifacts` and `--transport-harden-ssh` materialize the private key under the client realm, write a pinned `known_hosts` file under the client realm, and generate a realm-local SSH config snippet. The emitted command uses `ssh -F <realm ssh config> <alias>`. | This keeps hosting-owned SSH material isolated from the operator's normal SSH profile. Editing global `~/.ssh/config` would create cross-application side effects, host alias collisions, and rollback/ownership ambiguity. | Yes, but only as an explicit opt-in import/merge feature with managed marker blocks, conflict detection, and rollback. The current safer closure is already implemented: a self-contained `-F` config path. |
| User-scoped `authorized_keys` | `--transport-install-authorized-key` installs the transport public key into `~/.ssh/authorized_keys` by default, or `--ssh-authorized-keys-file` if provided. It writes a managed marker block, defaults to a forced command of `python -m hosting.engine_host_cli --relay-wrapper`, disables PTY/agent/X11/port forwarding, and registers the same key in hosting RBAC as role `transport`. `--transport-harden-ssh` composes this with client artifact provisioning and validation. | This is intentionally user-scoped because it does not require root/admin rights and it constrains the key to the hosting relay wrapper instead of granting shell access. It still depends on the target account's SSH login being allowed by machine policy. | Mostly closed already for the supported relay posture. Remaining possible closures are diagnostics and repair around permissions, existing conflicting key lines, or per-account SSH login policy; unrestricted shell or port-forward keys should stay outside the baseline. |
| Machine-wide SSH service/firewall | `--transport-admin-setup` generates a platform-specific elevated script by default and only executes it with `--admin-setup-execute`. It can enable/start OpenSSH service, add a supported firewall allowance when `--admin-setup-enable-firewall` is set, and enable systemd user linger when requested. | These are machine-wide changes, so the tool keeps them behind an explicit admin path and uses UAC, macOS authorization, `pkexec`, or `sudo` rather than collecting or storing administrator credentials. The generated script only covers common service/firewall helpers. | Partially. The script can reasonably add more named, auditable admin setup operations, but it should keep dry-run-by-default and explicit execution. It should not silently mutate broad host policy. |
| Arbitrary `sshd_config` policy | The script does not edit arbitrary `sshd_config` settings. | `sshd_config` is host-global and distro/vendor managed. A small change can lock out administrators, weaken existing policy, or conflict with fleet configuration management. Correct edits require parsing include trees, `Match` blocks, platform defaults, validation with `sshd -t`, restart/reload semantics, and rollback. | Technically yes, but only for narrow, named policies with validation and rollback. Broad arbitrary policy editing should remain outside this script and be handled by the host's normal admin/config-management process. |
