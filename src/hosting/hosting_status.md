...
- [ ] Add a minimal workflow JS helper executor on the existing hosting worker/sandbox model.
- [ ] Prefer a specialization of the generic worker path before adding a fully separate lifecycle manager.
- [ ] Use `worker_profile_class = "generic"` unless implementation proves a narrower profile class is required.
- [ ] Use `executor_kind = "workflow_js_helper"` for persisted registrations and routing.
- [ ] Use sandbox profile `workflow_js_helper_v1`.
- [ ] Use execution contract `hosting.workflow_helper.worker.v1`.
- [ ] Spawn through `EngineHostService.spawn(...)` with a normal persisted worker registration.
- [ ] Persist `sandbox_policy`, `sandbox_runtime`, command, environment, worker id, IPC family/address, auth token metadata, and capabilities as existing workers do.
- [ ] Expose status/diagnostics through existing worker registration/runtime status surfaces.
- [ ] Report workflow JS helper availability, Node executable path, Node version, worker id, engine id, sandbox profile, launch mode, platform sandbox status, and current capacity/busy state when available.
- [ ] Add ensure-running/shutdown/status behavior using existing worker lifecycle APIs.

## Workflow JS Helper RPC Contract

- [ ] Implement RPC method `execute_workflow_js_helper`.
- [ ] Accept `module_source` as the public source input.
- [ ] Do not accept caller-provided JS file paths.
- [ ] Verify `sha256(module_source) == module_sha256` before execution.
- [ ] Execute only the requested named export.
...
- [ ] Internally materialize source into host-owned staging files only if the Node execution strategy requires files.
- [ ] Keep host-owned staging paths out of the public request contract.
- [ ] Enforce per-call timeout from `limits.timeout_ms`.
- [ ] Enforce output size from `limits.output_limit_bytes`.
- [ ] Report memory limit behavior from `limits.memory_limit_mb`, including whether enforcement is active, best-effort, or unavailable on the current platform.
- [ ] Add safe concurrent-call handling through a bounded serialized queue, bounded worker pool, or explicit `workflow_sandbox_capacity_exceeded`/busy response.
- [ ] Include runtime details in success and failure results: worker id, engine id, Node version, and sandbox profile.

## Workflow JS Helper Sandbox Policy

- [ ] Use `WorkerSandboxPolicy` for the JS helper worker.
- [ ] Set default v1 policy to sandbox enabled.
- [ ] Set default v1 sandbox profile to `workflow_js_helper_v1`.
- [ ] Set process subprocess spawning to disabled.
- [ ] Set network mode to disabled.
- [ ] Disable brokered filesystem for helper-visible access.
- [ ] Disable brokered HTTP.
- [ ] Disable brokered subprocess.
- [ ] Ensure helper JS has no filesystem capability even if the host internally stages module files.
- [ ] Document that v1 has no direct network, brokered network, brokered filesystem, helper subprocess, general Node app hosting, or long-running workflow jobs.

## Workflow JS Helper Result And Error Mapping

- [ ] Return success shape with `ok: true`, `result`, and `runtime`.
- [ ] Return failure shape with `ok: false`, `reason`, `detail`, and `runtime`.
...
- [ ] Carry package id through request handling, worker logs, and host-side diagnostics.
- [ ] Carry workflow id through request handling, worker logs, and host-side diagnostics.
- [ ] Carry package source digest.
- [ ] Carry module SHA-256.
- [ ] Carry operation and export name.
- [ ] Carry session id, context id, cursor id, and workflow root id when provided.
...
- [ ] Evaluate whether `hosting.engine_worker_ipc` can host this contract without inheriting model-worker assumptions.
- [ ] If generic worker can support it cleanly, add a contract dispatch path keyed by `MP13_WORKER_CONTRACT` or executor kind.
- [ ] If generic worker cannot support it cleanly, add the smallest new worker module, likely `hosting.workflow_js_helper_ipc`, while still using shared spawn/sandbox/IPC infrastructure.
- [ ] Avoid routing workflow helper calls through model tool APIs.
- [ ] Avoid forcing workflow helpers into logical toolbox state unless they are intentionally exposed as toolbox tools.
- [ ] Keep the public host API executor-kind based so Python and JS helpers can share lifecycle, status, and shutdown patterns.
...
- [ ] Update [sandbox/GENERIC_WORKER.md](sandbox/GENERIC_WORKER.md) with the generic-worker extension decision and workflow helper relationship.
...
- [ ] Add a workflow helper worker doc if a separate executor module is introduced.
- [ ] Document the neutral runtime environment root and the `toolbox_venvs` migration/compatibility rule.
- [ ] Document default workflow JS sandbox policy and v1 non-goals.
- [ ] Document the workflow JS helper RPC request/result/error contract.
- [ ] Update [HOSTING_CLIENT_BREAKING_CHANGES.md](HOSTING_CLIENT_BREAKING_CHANGES.md) with dependent-project migration guidance for this work.
- [ ] In [HOSTING_CLIENT_BREAKING_CHANGES.md](HOSTING_CLIENT_BREAKING_CHANGES.md), list what dependent projects must stop doing, including direct Node helper spawning, caller-provided helper file paths, reliance on `toolbox_venvs` naming, and treating runtime Python bootstrap as a permanent fallback.
- [ ] In [HOSTING_CLIENT_BREAKING_CHANGES.md](HOSTING_CLIENT_BREAKING_CHANGES.md), list what dependent projects should start doing, including calling the workflow helper RPC with `module_source`, using `executor_kind = "workflow_js_helper"`, relying on host-managed sandbox policy/lifecycle/status, and reading neutral runtime environment metadata instead of toolbox-specific paths.
- [ ] In [HOSTING_CLIENT_BREAKING_CHANGES.md](HOSTING_CLIENT_BREAKING_CHANGES.md), include a before/after migration example for mp13-docs workflow JS helpers.
...
- [ ] Unit test JS module identity verification.
- [ ] Unit test JS allowed operation filtering.
- [ ] Unit test JS missing export behavior.
- [ ] Unit test JSON input/output validation.
- [ ] Unit test timeout and output limit behavior.
- [ ] Unit test memory limit reporting.
- [ ] Unit test capacity/busy behavior under concurrent calls.
- [ ] Integration test JS helper worker spawn, RPC execution, status, ensure-running, and shutdown.
- [ ] Integration test sandbox policy persistence and sandbox runtime reporting for `workflow_js_helper`.
...
- [ ] New workflow helper support uses existing host spawn, sandbox policy, launcher, persisted registration, IPC/RPC, lifecycle, and status mechanisms.
...
- [ ] Workflow JS helpers execute from `module_source`, never caller-provided file paths.
- [ ] Workflow JS helpers have no helper-visible filesystem, network, brokered I/O, or subprocess access in v1.
- [ ] Workflow Python and JS helper metadata carry enough provenance for security review.
- [ ] The project no longer describes verified runtime selection as generic fallback compatibility code.
- [ ] [HOSTING_CLIENT_BREAKING_CHANGES.md](HOSTING_CLIENT_BREAKING_CHANGES.md) clearly tells dependent projects what to stop doing and what to start doing as a result of this task.
