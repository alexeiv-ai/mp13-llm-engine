...
2. The default forced command is `python -m hosting.engine_host_cli --relay-wrapper`.
...
```text
command="python -m hosting.engine_host_cli --relay-wrapper",no-pty,no-agent-forwarding,no-X11-forwarding,no-port-forwarding ssh-ed25519 <key> <comment>
```
...
2. `python -m hosting.engine_host_cli --relay-wrapper`
