...
- [x] Constrain long-lived recovery to explicit host-managed state plus crashed-request artifact handoff.
  ...
  - Implemented behavior: failed workflow requests with declared artifact outputs retain their run folder and return an `artifact_recovery` notice. Clients can inspect candidates and normally claim selected outputs into `@artifacts/instances/<instance_id>/...`; explicit `target_id` remains an override for non-instance recovery flows.
