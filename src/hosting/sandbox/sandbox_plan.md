...
5. Decide whether to add `toolbox.cancel`.
...
4. small operator UX polish only if still needed after that
5. stop and review whether env/provenance depth is worth more investment
6. only then take on larger backend/platform work
...
4. operator UX is already mostly good, so only small targeted polish should remain after the core contract is aligned
5. deeper env/provenance work can become expensive quickly
6. Linux and bigger runtime adoption are meaningful, but not needed to validate the current Windows-first hosted-toolbox architecture
