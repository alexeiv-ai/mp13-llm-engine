...
def summarize_effective_tool_view(
    ...
    *,
    ...
    local_gated = _normalized_name_set(tools_view.gated_tools)
    ...
    if hosted_allowed:
        ...
        effective_gated = local_gated & hosted_allowed
        local_known = local_advertised | local_hidden | local_disabled | local_gated
        hosted_gated = (((local_known - local_gated) & hosted_allowed) - effective_advertised - effective_hidden) | (
            (local_advertised | local_hidden | local_gated) - hosted_allowed
        )
        hosted_gated -= effective_gated
    else:
        effective_advertised = set(local_advertised)
        effective_hidden = set(local_hidden)
        effective_gated = set(local_gated)
        hosted_gated = set()
    return {
        ...
        "effective_gated_tools": sorted(effective_gated),
        "effective_hidden_gated_tools": sorted(effective_gated - effective_advertised),
...
def annotate_tool_listing(
    ...
    *,
    ...
    local_gated = _normalized_name_set(tools_view.gated_tools) if tools_view else set()
    ...
    for item in list(listed_tools or []):
        ...
        scope_gated = tool_name in local_gated if tools_view else False
        if hosted:
            if tool_name in hosted and scope_allowed:
                availability = "Yes"
                via = "hosted-hidden" if tool_name in hosted_hidden else "hosted"
            elif tool_name in hosted and scope_gated:
                availability = "No"
                via = "gated"
