...
async def _tool_execution_action_handler(
    ...
    **kwargs: Any
    ...
    if execute_stage == 'call_starting' and tool_call:
        ...
        if tools_view is not None and tool_name:
            known_in_view = (
                tool_name in set(tools_view.advertised_tools or set())
                or tool_name in set(tools_view.hidden_allowed_tools or set())
                or tool_name in set(tools_view.allowed_tools or set())
                or tool_name in set(tools_view.gated_tools or set())
            )
            pending_approval = tools_view.is_gated(tool_name)
        ...
        if toolbox and toolbox.is_executable(tool_name, tools_view=tools_view):
            ...
            print(f"\n  {Colors.TOOL_WARNING}--- Executing Tool (pending approval): {tool_name}{batch_context} ---{Colors.RESET}")
            ...
            print(f"\n  {Colors.TOOL_WARNING}--- Executing Hosted Tool: {tool_name}{batch_context} ---{Colors.RESET}")
            print(f"    {Colors.TOOL_WARNING}Tool:     {Colors.RESET} {tool_name}")
            print(f"    {Colors.TOOL_ARGS}Arguments:{Colors.RESET} {Colors.DIM}{tool_args_str}{tool_args_note}{Colors.RESET}")
            if recovered_preview:
                print(f"    {Colors.TOOL_ARGS}Recovered expr preview:{Colors.RESET} {Colors.DIM}{recovered_preview}{Colors.RESET}")
...
async def main_logic():
    ...
    parser.add_argument(
        "--hosted-demo-project-file-peek-scope-root",
        type=str,
        default=None,
    ...
    if args.hosted_demo:
        ...
        if args.hosted_demo_project_file_peek_scope_root is not None:
